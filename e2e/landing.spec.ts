...
test.describe('Landing page', () => {
  ...
  test('navigates to /app when CTA button is clicked', async ({ page }) => {
    ...
    const ctaBtn = page
      .locator('a[href*="/app"], button')
      .filter({ hasText: /start|starten|extract|öffnen/i })
      .first()
    await ctaBtn.click()
  ...
  test('theme toggle button is present', async ({ page }) => {
    ...
    const themeBtn = page
      .locator(
        'button[aria-label*="theme"], button[aria-label*="Theme"], button[title*="theme"], button[class*="theme"]'
      )
      .first()
